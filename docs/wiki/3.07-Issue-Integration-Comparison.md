...
| Gitea           | Yes    | No            | No         | No       | No       | No          | Repo, scope, labels  | Up to 200   | —         | —         |
...
- **Due date:** Optional. Setting or removing a task deadline writes the Jira due date (date only).
...
- **Auto-import:** Up to 200 issues for the project (4 pages × up to 50 per page; subject to server-imposed per-page limits).
...
- **Card creation:** Supported when a default Trello list is configured.
...
- **Title:** Configurable field mapping. Title updates pull from Trello by default.
...
- **Title & schedule:** **Pull only** — a rename or reschedule made in Plainspace pulls in on the next poll (`scheduledAt` → `dueWithTime`), but renaming or rescheduling the task locally is **not** pushed back. **Recurring** tasks are flagged in the claim pool and task panel; the Plainspace server advances the schedule to the next occurrence.
- **Recurring tasks:** A recurring Plainspace task stays **one** local task across all its occurrences — Plainspace keeps a single item and moves its schedule forward. Once the server has advanced to the next occurrence, the local task is reopened and rescheduled on the next poll; if you archived it in the meantime, it is restored from the archive as the next occurrence. Consequences worth knowing: notes and subtasks carry over, **completed subtasks stay completed** (only the parent reopens), **tracked time accumulates across all occurrences** rather than starting fresh (unlike Super Productivity's own repeating tasks), and the completed occurrence leaves no separate archive entry. Archiving does not stop a recurring task from coming back, and neither does deleting it (it is re-imported on the next poll) — end or unassign the repetition in Plainspace instead. For the same reason, avoid setting a **time estimate** on a recurring Plainspace task: it is compared against the accumulated total, so once the accumulated time reaches the estimate, a task without subtasks counts as zero remaining time in the planner and schedule on later occurrences. Per-day tracking (worklog, daily summary, time exports) stays correct either way.
- **Worklog / Comments / Subtasks / Attachments:** Not supported.
...
- **Disconnecting the account:** On an unlinked project, open **Collaborate on Plainspace → Disconnect account** in the space picker. This removes the saved account from this device and closes the dialog. Opening collaboration again, including after restarting Super Productivity, asks you to connect again. Existing project integrations retain their own credentials and keep syncing; to stop them, open each relevant Plainspace integration in the **issue provider panel**, then choose its **gear icon → Delete**. Disconnecting does not revoke the API token or sign you out of the Plainspace website. Editing or deleting a provider, or restoring a backup, does not clear the separate saved account.
...
- **Collaboration vs. import:** Plainspace is collaboration-oriented — done state syncs both ways, title and schedule pull in from Plainspace, and you can share a local project as a shared Space. Other providers are primarily imports from external trackers, though some fields can be configured for two-way updates where supported, such as Trello card status and title.
